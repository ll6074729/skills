...
- 用户要求为特定技术栈定制规范文档
- 用户要求整合技术最佳实践与项目规范
- 用户要求集成本地 SKILL.md 技能资源到规范生成流程
- 用户要求优化、修订或对齐已有项目规范

## 二、核心执行流程

本技能严格按以下七个阶段顺序执行，不得跳过任何阶段：

```mermaid
flowchart LR
    Z[阶段零 任务意图选择与本地技能集成] --> A[阶段一 动态技术栈确认]
    A --> B[阶段二 深度检索最佳实践]
    B --> C[阶段三 确认规范范围]
    C --> D[阶段四 生成规范文档]
    D --> E[阶段五 自检与交付]
...
**设计理念**：本阶段为流程的最先入口，确保用户每次触发本技能时都被显式询问任务意图，避免在 IDE / Agent 宿主中意图被默认假设而跳过。同时提供本地技能集成机制，允许用户将本地存储的 SKILL.md 资源注入到后续阶段作为参考素材，与 WebSearch 检索结果合并使用。

**强制要求**：

1. **必须作为最先触发的入口步骤**：在执行任何项目预检、WebSearch 检索或 AskUserQuestion 之前，必须先完成本阶段
2. **禁止假设任务意图**：即使宿主环境（如 IDE / Agent）未显式传递意图参数，也必须通过`AskUserQuestion`询问用户
3. **本地技能集成是可选的**：用户可选择不集成本地技能，直接进入阶段一
4. **本地技能格式限定为 SKILL.md**：必须是符合本技能同款 YAML frontmatter + Markdown 正文格式的本地 SKILL.md 文件
5. **本地技能以「参考素材」身份注入**：本地技能内容不作为新规则域独立写入，仅在阶段二深度检索时与 WebSearch 结果合并为参考素材，规范文档引用本地技能条目作为来源

#### 0.1 任务意图选择（强制第一步）

通过`AskUserQuestion`询问用户任务意图，提供以下选项：

1. 新生成项目规范（推荐）- 为新项目或现有项目从零生成完整规范体系
2. 优化已有项目规范 - 项目已有 AGENTS.md 或 .agents/rules/，需要补充、修订或对齐最新指南
3. 集成本地技能并生成规范 - 在生成规范时注入用户本地 SKILL.md 资源作为参考素材
4. 仅推荐 Skills - 跳过规范生成，仅基于项目技术栈从 skills.sh 检索并输出推荐 Skills 列表

**意图对后续流程的影响**：

| 任务意图 | 阶段一 | 阶段二 | 阶段三 | 阶段四 | 阶段五 | 阶段六 |
|----------|--------|--------|--------|--------|--------|--------|
| 新生成项目规范 | 全流程 | 全流程 | 全流程 | 全流程 | 全流程 | 全流程 |
| 优化已有项目规范 | 复述已有规范 → 仅对未对齐维度询问 | 全流程（重点检索变更点） | 全流程（增量确认） | 仅生成差异文件 | 全流程 | 全流程 |
| 集成本地技能并生成规范 | 全流程，且阶段零加载本地技能 | 本地技能内容作为参考素材注入 | 全流程 | 全流程（标注本地技能来源） | 全流程（验证本地技能引用完整性） | 全流程 |
| 仅推荐 Skills | 跳过技术栈维度询问，仅做轻量预检 | 跳过 | 跳过 | 跳过 | 跳过 | 全流程 |

#### 0.2 本地技能集成入口（条件触发）

**触发条件**：用户在 0.1 选择了「集成本地技能并生成规范」意图。

**询问方式**：通过`AskUserQuestion`询问用户本地技能的位置与加载方式，提供以下选项：

1. 指定本地 SKILL.md 文件路径（推荐）- 用户直接提供一个或多个本地 SKILL.md 文件绝对路径
2. 指定本地技能目录 - 用户提供一个目录路径，系统递归扫描该目录下所有 SKILL.md 文件
3. 暂不集成本地技能 - 取消本地技能集成，回退为「新生成项目规范」意图流程
4. （Other：用户自定义输入其他加载方式）

#### 0.3 本地技能加载与解析（强制流程）

**强制要求**：用户指定本地技能路径后，必须按以下流程加载与解析，不得跳过任何子步骤：

1. **路径校验**：使用`Glob`或`LS`校验用户提供的路径是否存在
   - 文件路径：必须是扩展名为`.md`的文件，且文件名推荐为`SKILL.md`
   - 目录路径：递归扫描目录下所有`SKILL.md`文件（不区分大小写），忽略其它扩展名
2. **格式校验**：使用`Read`读取每个本地 SKILL.md 文件，校验是否包含 YAML frontmatter（以`---`开头并以`---`结束的首部块）
   - 必含字段：`name`、`description`
   - 若缺失 frontmatter：明确告知用户该文件不符合 SKILL.md 格式，跳过该文件并提示用户修正
3. **元信息提取**：从 frontmatter 提取以下元信息，整合为「本地技能清单」：
   - `name`：技能名称
   - `description`：技能描述（用于阶段二的参考素材匹配）
   - 文件绝对路径
   - 文件大小（用于阶段二的素材权重评估）
4. **正文摘要**：读取 frontmatter 之后的 Markdown 正文，提取以下结构化摘要：
   - 顶级章节标题列表（`^## ` 开头的行）
   - 触发场景段落（若正文中包含「触发」「何时」等关键词的段落）
   - 关键约束段落（若正文中包含「禁止」「强制」「必须」等关键词的段落）
5. **本地技能清单复述**：通过文本形式向用户复述已加载的本地技能清单（技能名、路径、描述摘要），请求用户确认是否继续
...
是否确认以上本地技能清单将作为参考素材注入后续流程？
```

#### 0.4 本地技能注入约定

**注入位置**：阶段二「深度检索各技术最佳实践与框架内置组件」。

**注入方式**：

1. **作为参考素材合并**：本地技能正文中的规则条目、约束条款、组件清单与阶段二的 WebSearch 检索结果合并为「技术最佳实践摘要」
2. **来源标注**：每条来自本地技能的素材必须标注来源为「本地技能：{技能名}（{绝对路径}）」，与 WebSearch 来源素材明确区分
3. **优先级**：本地技能素材与 WebSearch 素材同优先级；当两者冲突时，以《通用项目规范体系设计指南》原则为优先框架（参照 7.3 节）
4. **使用范围限定**：本地技能素材仅用于规范文档内容生成的素材层，不得修改《通用项目规范体系设计指南》的五大支柱、AI 行为规范、共性原则等框架性内容

**禁止事项**：

- 禁止将本地技能作为独立规则域写入 `.agents/rules/` 目录
- 禁止用本地技能内容覆盖《通用项目规范体系设计指南》原则
- 禁止在未通过 0.3 格式校验的情况下注入本地技能
- 禁止在阶段四生成的规范文档中省略本地技能来源标注

#### 0.5 阶段零交付物

阶段零结束时，必须形成以下交付物，供后续阶段引用：

1. **任务意图记录**：用户选择的任务意图（用于阶段一至六的流程裁剪）
2. **本地技能清单**（若用户选择集成）：包含每项本地技能的 name、description、绝对路径、章节摘要、关键约束条目数
3. **本地技能正文缓存**（若用户选择集成）：在主上下文中保留本地技能正文片段，供阶段二检索调用；若上下文容量不足，分片缓存并按需读取

#### 0.6 降级与异常处理

- 若用户提供的路径不存在：明确告知用户路径无效，请求重新提供或选择「暂不集成本地技能」
- 若本地 SKILL.md 格式不合法：明确告知用户哪些文件格式不合法，跳过这些文件并提示用户修正
- 若本地技能正文为空：明确告知用户该技能正文为空，仅注入 frontmatter 元信息
- 若加载过程中出现任何异常：必须告知用户具体异常原因，并提供「跳过本地技能集成」的降级路径
...
- 前端框架：Vue 3.x + Ant Design Vue（来自 package.json + vite.config.ts）
...
- 若用户表示采用包含内置组件库的框架（如 ant-design-pro、element-plus-admin 等），优先推荐使用该框架的开源项目作为参考
...
收集内容参考 本项目 templates/framework-components.template.md 模式，包括：
...
若阶段零加载了本地技能清单，本阶段必须将本地技能正文素材与 WebSearch 检索结果合并为「技术最佳实践摘要」：

1. **素材合并**：将本地技能正文中的规则条目、约束条款、组件清单与 WebSearch 检索结果按相同主题归并
2. **来源标注**：每条来自本地技能的素材必须标注来源为「本地技能：{技能名}（{绝对路径}）」，与 WebSearch 来源素材明确区分
3. **优先级处理**：本地技能素材与 WebSearch 素材同优先级；冲突时以《通用项目规范体系设计指南》原则为优先框架
4. **使用范围限定**：本地技能素材仅用于规范文档内容生成的素材层，不得修改指南的五大支柱、AI 行为规范、共性原则等框架性内容
...
4. **结果整合**：将检索到的最佳实践要点与本地技能素材合并，整理为「技术最佳实践摘要」，包含：
   - 官方推荐的项目目录结构
   - 命名约定与代码风格
   - 依赖注入与模块组织方式
   - 测试策略与覆盖率要求
...
检索完成后，必须将框架内置组件清单整理为标准化表格（参考 本项目 templates/framework-components.template.md 的「组件使用要求」章节）：

| 功能场景 | 必须使用的内置组件 | 配置方式 | 禁止替代方案 |
|----------|-------------------|----------|--------------|
| {场景，如表格页面} | {组件名，如`{表格组件}`} | {配置方式} | {禁止，如自行封装表格} |
| {场景，如弹窗} | {组件名，如`{弹窗组件}`} | {配置方式} | {禁止} |
...
1. **前端 UI 规则**（`frontend-ui.md`）必须包含「组件使用要求」章节，明确列出：
   - 表格场景必须使用的组件（参考 本项目 templates/framework-components.template.md 的 `{表格组件}` 模式）
   - 表单场景必须使用的组件（如 `{表单组件}`）
   - 弹窗场景必须使用的组件（如 `{弹窗组件}`）
   - 抽屉场景必须使用的组件（如 `{抽屉组件}`）
   - 操作列必须使用的组件（`{操作列组件}`、`{下拉菜单组件}`、`{确认组件}`，由 WebSearch 实时检索填充）
   - 上传下载必须使用的方式（`{上传组件}`、`{下载方法}`，由 WebSearch 实时检索填充）
   - 图标必须使用的组件和命名格式（`{图标组件}` + `{图标命名格式}`，由 WebSearch 实时检索填充）
   - 全局组件注册位置（`{全局组件目录}`，由 WebSearch 实时检索填充）
   - 适配器层目录结构（`{适配器层目录}`，由 WebSearch 实时检索填充）
...
每个规则文件的「框架内置组件优先使用」章节必须采用以下结构（参考 本项目 templates/framework-components.template.md 的模式）：
...
- {场景}必须使用`{组件名}`（如`{表格组件}`）和`{配套组件}`（如`Page`）。
...
1. **任务意图摘要**：复述阶段零确认的任务意图（新生成/优化已有/集成本地技能/仅推荐 Skills）
2. **文件清单**：列出所有生成的文件路径（使用 clickable file links）
3. **技术栈摘要**：复述用户提供的所有技术栈信息
4. **最佳实践来源**：列出检索到的每项最佳实践来源链接
5. **本地技能清单与引用**（若用户选择集成）：列出本地技能 name、绝对路径、章节摘要、在规范文档中的引用位置（章节级），便于用户回溯
6. **待定项说明**：标注所有`待定`项及建议后续补充方式
7. **使用指南**：简要说明如何使用生成的规范体系
8. **检查清单报告**：输出自检清单的完成情况
9. **Skills 推荐预告**：告知用户阶段六将通过子智能体并行搜索适合项目的 skills，并更新 README.md
...
1. **技术栈信息**：已确定的技术栈（如「NestJS+TypeORM+Vue3+Ant Design Vue」）
...
- 代码语法和代码块内容（如`{表单组件}`、`@Injectable`、`g.DB()`等）
...
**强制要求**：当用户在阶段零选择集成本地技能时，必须严格遵守以下约束：

- 禁止跳过阶段零 0.3 的格式校验直接注入本地技能
- 禁止将本地技能作为独立规则域写入 `.agents/rules/` 目录
- 禁止用本地技能内容覆盖《通用项目规范体系设计指南》的五大支柱、AI 行为规范、共性原则等框架性内容
- 禁止在阶段四生成的规范文档中省略本地技能来源标注（必须标注「本地技能：{技能名}（{绝对路径}）」）
- 禁止在未通过 0.3 格式校验的情况下注入本地技能（frontmatter 缺失、必填字段缺失等）
- 禁止假设本地技能内容已完整覆盖某技术领域，必须与 WebSearch 检索结果交叉验证
- 本地技能素材与 WebSearch 素材冲突时，必须以《通用项目规范体系设计指南》原则为优先框架，并明确告知用户冲突点
...
本章节详细说明如何根据项目所使用的具体组件库或开发插件（如 ant-design-pro、element-plus-admin、nest-admin 等）进行动态适配调整，确保规范体系能够灵活适应不同技术栈环境的需求。

### 5.1 动态适配触发条件

以下任一条件命中时，必须启动框架内置组件动态适配：

- 用户在阶段一表示采用了包含内置组件库的框架（ant-design-pro、element-plus-admin、nest-admin 等，由 WebSearch 实时检索确认）
- 用户提供了框架官方文档链接
- 用户提供了参考项目（如 {参考项目}）
...
检索关键词模板：
- `{框架名} built-in components`（框架名由阶段一用户确认或预检识别）
...
**适配内容**（参考 本项目 templates/framework-components.template.md 模式）：

| 适配项 | 适配方式 | 示例 |
|--------|----------|------|
| 表格组件 | 列出框架内置表格组件名 | 由 WebSearch 实时检索填充 |
| 表单组件 | 列出框架内置表单组件名 | 由 WebSearch 实时检索填充 |
| 弹窗组件 | 列出框架内置弹窗组件名 | 由 WebSearch 实时检索填充 |
| 抽屉组件 | 列出框架内置抽屉组件名 | 由 WebSearch 实时检索填充 |
| 操作列组件 | 列出操作列组件组合 | 由 WebSearch 实时检索填充 |
| 上传组件 | 列出框架内置上传组件 | 由 WebSearch 实时检索填充 |
| 下载方法 | 列出框架内置下载方法 | 由 WebSearch 实时检索填充 |
| 图标组件 | 列出图标组件和命名格式 | 由 WebSearch 实时检索填充 |
...
- 若用户提供的本地技能路径不存在：明确告知用户路径无效，请求重新提供或选择「暂不集成本地技能」回退流程
- 若本地 SKILL.md 格式不合法（frontmatter 缺失、必填字段缺失）：明确告知用户哪些文件格式不合法，跳过这些文件并提示用户修正
- 若本地技能正文为空：仅注入 frontmatter 元信息，并在交付清单中标注「正文为空」
- 若本地技能内容与《通用项目规范体系设计指南》原则冲突：以指南原则为优先框架，明确告知用户冲突点并保留本地技能来源标注
- 若本地技能加载或解析过程出现任何异常：必须告知用户具体异常原因，并提供「跳过本地技能集成」的降级路径，不阻塞主流程
...
**用户输入**：「为我们的新项目生成规范，我们用 ant-design-pro 框架」

**执行流程**：

1. 阶段一：动态技术栈确认
   - **1.0 项目技术栈预检**：Glob 扫描项目根目录 → 若识别到 package.json 中包含 @ant-design/pro-components 依赖 → 复述确认前端框架为 ant-design-pro
   - 维度一：WebSearch 检索当前主流后端框架 → 动态生成选项 → 询问用户（用户选 Other 自定义或选择推荐项）
   - 维度二：前端框架已通过预检识别为 ant-design-pro → 复述确认 → 不再询问
   - 维度三至六：检索并询问数据库、ORM、工程化栈、基础设施
   - 维度七：WebSearch 检索「ant-design-pro built-in components」→ 确认 ant-design-pro 包含完整内置组件库 → 询问框架文档链接和参考项目
   - 开源项目推荐：WebSearch 检索「ant-design-pro open source project github」→ 推荐使用 ant-design-pro 框架的高质量开源项目 → 询问用户是否采用参考项目
2. 阶段二：深度检索
   - 检索 ant-design-pro 框架内置组件清单（由 WebSearch 实时检索填充）
   - WebFetch 获取 ant-design-pro 框架官方文档内容
   - 深度分析参考项目的组件使用约定
   - 整理标准化组件清单表（参考 本项目 templates/framework-components.template.md 模式）
3. 阶段三：确认输出位置、输出形式、规则域、语言
4. 阶段四：
   - 读取指南文件
   - 生成 AGENTS.md
   - 生成 `frontend-ui.md`，包含「组件使用要求」章节，明确列出检索到的 ant-design-pro 内置组件
   - 生成 `backend.md`，包含「框架内置能力优先使用」章节
   - 生成其他规则文件
5. 阶段五：自检清单（含框架内置组件适配验证）、交付文件清单、技术栈摘要、最佳实践来源、ant-design-pro 组件清单来源、待定项说明
6. 阶段六：汇总子智能体从 skills.sh 检索的 ant-design-pro 相关 skills，更新 README.md（含「推荐 Skills」章节）
...
**用户输入**：「生成规范，并集成本地技能，本地技能在 `/path/to/local-skills/` 目录下」

**执行流程**：

1. 阶段零：任务意图选择与本地技能集成
   - **0.1 任务意图选择**：AskUserQuestion 询问 → 用户选择「集成本地技能并生成规范」
   - **0.2 本地技能集成入口**：AskUserQuestion 询问 → 用户选择「指定本地技能目录」并提供路径 `/path/to/local-skills/`
   - **0.3 本地技能加载与解析**：
     - LS 递归扫描 `/path/to/local-skills/` 目录 → 找到 `SKILL.md`、`sub-skill/SKILL.md` 等 N 个文件
     - Read 读取每个文件 → 校验 frontmatter 含 `name` 和 `description` 字段
     - 提取每项技能的 name、description、章节摘要、关键约束条目数
     - 向用户复述本地技能清单 → 用户确认继续
2. 阶段一：动态技术栈确认
   - 按既有 1.0 至 1.7 流程执行技术栈确认（用户集成的本地技能不影响阶段一流程）
3. 阶段二：深度检索最佳实践
   - **本地技能素材注入**：将本地技能正文中的规则条目、约束条款、组件清单与 WebSearch 检索结果合并为「技术最佳实践摘要」
   - 每条来自本地技能的素材标注来源「本地技能：{技能名}（/path/to/local-skills/SKILL.md）」
   - 本地技能素材与 WebSearch 素材冲突时，以《通用项目规范体系设计指南》原则为优先框架，并明确告知用户冲突点
4. 阶段三至四：按既有流程确认范围并生成规范文档，规范文档对应章节标注本地技能来源
5. 阶段五：自检清单（含本地技能集成验证）、交付文件清单、技术栈摘要、最佳实践来源、本地技能清单与引用位置、待定项说明
6. 阶段六：子智能体从 skills.sh 检索适合的 skills，更新 README.md

## 九、技能依赖

本技能执行依赖以下工具：

- `Glob`：扫描项目根目录配置文件，执行项目技术栈预检（阶段一 1.0 前置步骤）；校验用户在阶段零提供的本地技能文件路径是否存在
- `LS`：检查项目目录结构，推断技术栈（阶段一 1.0 前置步骤）；递归扫描用户在阶段零提供的本地技能目录
- `AskUserQuestion`：询问用户任务意图（阶段零 0.1）、本地技能加载方式（阶段零 0.2）、技术栈、规范范围和框架内置组件清单
- `WebSearch`：检索各技术最新最佳实践和框架内置组件使用规范
- `WebFetch`：获取用户提供的框架文档 URL 内容，提取组件清单和使用方式
- `Read`：读取《通用项目规范体系设计指南》文件、项目配置文件、参考项目规范以及阶段零用户指定的本地 SKILL.md 文件，校验 frontmatter 与正文格式
- `Write`：生成规范文档文件
...
- 初始版本：v1.0
- v1.1 更新：新增框架内置组件动态适配机制，支持根据项目采用的组件库或开发插件生成「框架内置组件优先使用」规则
...
- v1.9 更新：新增阶段零「任务意图选择与本地技能集成」，作为流程最先入口显式询问用户任务意图（新生成/优化已有/集成本地技能/仅推荐 Skills），避免宿主环境默认假设意图；新增本地 SKILL.md 技能集成机制，支持文件路径或目录路径加载、frontmatter 格式校验、章节摘要提取、作为参考素材注入阶段二；彻底移除所有具体项目引用（特定前端框架、参考项目路径等），改为中性占位符 `{框架名}` `{表格组件}` `{表单组件}` `{弹窗组件}` `{抽屉组件}` `{参考项目}`，由 WebSearch 实时检索填充；移除模板文件中具体框架示例，统一改为「由 WebSearch 实时检索填充」
